...
class TextChatService {
  ...
  setUserContext(userId: string, sessionId?: string, appContext?: any): void {
    ...
    logger.debug('TEXT_CHAT_SERVICE', 'User context set', {
      userId,
      sessionId,
  ...
  async sendMessage(userMessage: string, stream: boolean = true): Promise<void> {
    ...
    try {
      ...
      if (this.userId) {
        ...
        logger.debug('TEXT_CHAT_SERVICE', 'User message persisted to database', {
          messageId,
          userId: this.userId,
      ...
      if (stream) {
        ...
        if (this.userId && accumulatedResponse.length > 0) {
          await conversationMemoryManager.saveMessage({
            userId: this.userId,
            sessionId: this.sessionId || undefined,
            role: 'assistant',
          ...
          logger.debug('TEXT_CHAT_SERVICE', 'Assistant message persisted to database', {
            messageId,
            userId: this.userId,
            responseLength: accumulatedResponse.length
          });
        ...
        // Persist assistant message to database
        if (this.userId && response.message.content.length > 0) {
          await conversationMemoryManager.saveMessage({
            userId: this.userId,
            sessionId: this.sessionId || undefined,
            role: 'assistant',
