...
class VoiceCoachOrchestrator {
  ...
  async startVoiceSession(mode: string): Promise<void> {
    ...
    try {
      ...
      if (!realtimeIntegration.isBrainReady()) {
        logger.warn('VOICE_ORCHESTRATOR', '⚠️ Brain not ready, initializing...');
        // Brain should be initialized by AppProviders, but double-check
        const userId = useUserStore.getState().user?.id;
        if (userId && !brainCore.isInitialized()) {
      ...
      // Build context-enriched system prompt using HEAD integration
      logger.info('VOICE_ORCHESTRATOR', '🧠 Building context-enriched prompt via HEAD system', { mode });
      const enrichedSystemPrompt = await realtimeIntegration.buildRealtimeSystemPrompt(
        modeConfig.systemPrompt,
        mode as any
      );

      logger.info('VOICE_ORCHESTRATOR', '✅ HEAD-enriched prompt ready', {
        baseLength: modeConfig.systemPrompt.length,
        enrichedLength: enrichedSystemPrompt.length,
        contextAdded: enrichedSystemPrompt.length - modeConfig.systemPrompt.length
      });

      // Get user language preference for voice selection
      const userProfile = useUserStore.getState().profile;
      const preferredLanguage = userProfile?.preferredLanguage || 'fr';
      const voiceModel = preferredLanguage === 'en' ? 'alloy' : 'alloy'; // Both use 'alloy' for now, OpenAI will adapt

      logger.info('VOICE_ORCHESTRATOR', '🗣️ Voice language selection', {
        preferredLanguage,
        voiceModel
      });

      // Connect to Realtime API with HEAD-enriched prompt
      logger.info('VOICE_ORCHESTRATOR', '🌐 Connecting to Realtime API via WebRTC with HEAD context');
      await openaiRealtimeService.connect({
        model: 'gpt-realtime-mini',
        voice: voiceModel,
        temperature: 0.8,
        maxTokens: 4096,
        instructions: enrichedSystemPrompt // HEAD-enriched prompt with full user context
      });
      logger.info('VOICE_ORCHESTRATOR', '✅ Realtime API connected with HEAD-enriched context');

      // Configure session with enriched prompt
      logger.info('VOICE_ORCHESTRATOR', '⚙️ Configuring session with VAD and HEAD context');
      await openaiRealtimeService.configureSession(enrichedSystemPrompt, mode as any);
      ...
      // Generate AI welcome message using HEAD context
      // Get user first name for greeting only
      const firstName = userProfile?.displayName?.split(' ')[0] || 'champion';

      // Simple greeting prompt - AI will generate natural welcome based on full HEAD context
      const greetingPrompt = `Salue ${firstName} de façon naturelle et accueillante en une phrase courte. Adapte ton message selon son contexte actuel (dernière activité, objectifs, progression).`;

      logger.info('VOICE_ORCHESTRATOR', '👋 Generating AI welcome message via HEAD context', { firstName, mode });
      openaiRealtimeService.sendTextMessage(greetingPrompt);
